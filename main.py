...
class PathfindingApp:
    ...
    def draw_grid(self):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                ...
                elif (r, c) == self.agent_pos: color = "cyan"
                elif (r, c) == self.goal: color = "purple"
                elif (r, c) in self.current_path: color = "green" # Green for Final Path 
                elif (r, c) in self.frontier_nodes: color = "yellow" # Yellow for Frontier Nodes
                elif (r, c) in self.visited_nodes: color = "red" # Red/Blue for Visited Nodes
    ...
    def replan(self):
        ...
        if self.algorithm_var.get() == "A*":
            path, visited, frontier, nodes, ms = a_star(self.grid, self.agent_pos, self.goal, h_func)
        else:
            path, visited, frontier, nodes, ms = greedy_search(self.grid, self.agent_pos, self.goal, h_func)
